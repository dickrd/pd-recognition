    ...
    with tf.name_scope(scope, "googlenet", [input_tensor]):
        with ops.arg_scope([layers.max_pool2d], padding='SAME'):
            ...
            with tf.variable_scope("inception_3a"):
                end_points['inception_3a'] = _get_inception_layer(end_points['pool1'], 64, 96, 128, 16, 32, 32)
            with tf.variable_scope("inception_3b"):
                end_points['inception_3b'] = _get_inception_layer(end_points['inception_3a'], 128, 128, 192, 32, 96, 64)

            end_points['pool2'] = layers.max_pool2d(end_points['inception_3b'], [3, 3], scope='pool2')

            with tf.variable_scope("inception_4a"):
                end_points['inception_4a'] = _get_inception_layer(end_points['pool2'], 192, 96, 208, 16, 48, 64)
            with tf.variable_scope("aux_logits_1"):
                end_points['aux_logits_1'] = _aux_logit_layer(end_points['inception_4a'], num_class, is_training)
            with tf.variable_scope("inception_4b"):
                end_points['inception_4b'] = _get_inception_layer(end_points['inception_4a'], 160, 112, 224, 24, 64, 64)
            with tf.variable_scope("inception_4c"):
                end_points['inception_4c'] = _get_inception_layer(end_points['inception_4b'], 128, 128, 256, 24, 64, 64)
            with tf.variable_scope("inception_4d"):
                end_points['inception_4d'] = _get_inception_layer(end_points['inception_4c'], 112, 144, 288, 32, 64, 64)
            with tf.variable_scope("aux_logits_2"):
                end_points['aux_logits_2'] = _aux_logit_layer(end_points['inception_4d'], num_class, is_training)
            with tf.variable_scope("inception_4e"):
                end_points['inception_4e'] = _get_inception_layer(end_points['inception_4d'],
                                                                  256, 160, 320, 32, 128, 128)

            end_points['pool3'] = layers.max_pool2d(end_points['inception_4e'], [3, 3], scope='pool3')

            with tf.variable_scope("inception_5a"):
                end_points['inception_5a'] = _get_inception_layer(end_points['pool3'],
                                                                  256, 160, 320, 32, 128, 128)
            with tf.variable_scope("inception_5b"):
                end_points['inception_5b'] = _get_inception_layer(end_points['inception_5a'],
                                                                  384, 192, 384, 48, 128, 128)

            end_points['pool4'] = layers.avg_pool2d(end_points['inception_5b'], [7, 7],
                                                    stride=1,
                                                    scope='pool4')
            end_points['reshape'] = tf.reshape(end_points['pool4'], [-1, 4096])
            end_points['dropout'] = layers.dropout(end_points['reshape'], dropout_keep_prob,
                                                   is_training=is_training)
            end_points['logits'] = layers.fully_connected(end_points['dropout'], num_class,
                                                          activation_fn=None,
                                                          scope='logits')
