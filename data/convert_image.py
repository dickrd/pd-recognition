...
def print_dataset_summary(name_wrote_count=None, store_path=None, read_mode=False):
    ...
    for item in sorted_kv:
        print "\t{0}, {1}\t|".format(str(item[0]).encode("utf-8"), item[1]),
        count += 1
        if count % 5 == 0:
            print ""
    print ""
...
def _main():
    ...
    if args.pre_process == "general":
        ...
        imdb = ImdbWikiUtil(regression=args.regression)
        load_image = load_image_data
        walk = imdb.walk
        generate_name = imdb.name
        print "Imdb pre-process using age as label."
    elif args.pre_process == "imdb-sex":
        imdb = ImdbWikiUtil(regression=args.regression, sex=True)
        load_image = load_image_data
        walk = imdb.walk
        generate_name = imdb.name
        print "Imdb pre-process using sex as label."
    elif args.pre_process == "wiki":
        wiki = ImdbWikiUtil(regression=args.regression, source_type='wiki')
        load_image = load_image_data
        walk = wiki.walk
        generate_name = wiki.name
        print "Wiki pre-process using age as label."
    elif args.pre_process == "wiki-sex":
        wiki = ImdbWikiUtil(regression=args.regression, sex=True, source_type='wiki')
        load_image = load_image_data
        walk = wiki.walk
        generate_name = wiki.name
    ...
    elif args.pre_process == "iog-test":
        iog = IogUtil(regression=args.regression, is_test=True)
        load_image = iog.load_img
        walk = iog.walk
        generate_name = iog.name
        print "ImageOfGroups pre-process using age as label (test data)."
    elif args.pre_process == "iog-sex-test":
        iog = IogUtil(regression=args.regression, sex=True, is_test=True)
        load_image = iog.load_img
        walk = iog.walk
        generate_name = iog.name
